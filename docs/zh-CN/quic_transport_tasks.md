...
P1（Interop + Policy）、P2（API + 可维护性）、P3（Extensions + Ops）、P5（生产可用）全部 Done。1883 测试通过。

### P6 任务矩阵 — 高级功能与完整生态

P6 进入条件：P0-P5 完成，1883 测试通过。
...
| P6-C | 高级拥塞控制 | CUBIC 生产加固；算法切换 API | NewReno/CUBIC 连接启动时可选；均通过丢包/恢复测试 | Done |
...
| 标准领域 | 状态 |
| --- | --- |
| RFC 8999 版本无关属性 | Done |
| RFC 9000 传输协议 | Done |
| RFC 9001 TLS 与包保护 | Done |
| RFC 9002 丢包检测与拥塞控制 | Done |
| RFC 9221 DATAGRAM | Done |
| RFC 9368 Compatible Version Negotiation | Done |
| RFC 9369 QUIC v2 | Done |
| HTTP/3 与 QPACK | Done |

状态已按当前功能矩阵对齐（2026-08）；见 [feature_comparison.md](feature_comparison.md)。
剩余缺口仅为平台特有的 FIPS 140-3 与 XDP 零拷贝两项。
...
独立文档：[feature_comparison.md](feature_comparison.md)
