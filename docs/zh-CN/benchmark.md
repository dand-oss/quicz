...
| 指标 | 数值 | 说明 |
|---|---|---|
| 流上传（16 MB） | **280.40 MB/s** | 线程化 client/server，CUBIC，8.9KB datagram |
...
| 百分位 | 延迟 | 说明 |
|---|---|---|
| P50 | **21.5 μs** | 1 KB 完整 QUIC 往返（加密+发送+接收+解密+回显） |
| P99 | **46.7 μs** | |
| P99.9 | **126.8 μs** | |
...
| UDP loopback（线程化） | **272.58 MB/s** | std.Io 线程化，8.9KB datagram |

## 丢包恢复（loopback + 模拟丢包）

| 丢包率 | 吞吐量 | 保持率 | 说明 |
|---|---|---|---|
| 0% | 280.40 MB/s | 100% | 基线 |
...
| **quicz** | **Zig** | **~280 MB/s** | **macOS, loopback, 8.9KB datagram, 无 GSO** | **本基准** |
...
| **quicz** | **Zig** | **21.5 μs** | **46.7 μs** | **macOS, loopback, std.Io** | **本基准** |
...
| **quicz** | **280 MB/s** | **120.36 MB/s (43%)** | **37.48 MB/s (13%)** | **CUBIC** | **本基准** |
...
| **quicz** | **272.58 MB/s** | **共享 cwnd** | **本基准** |

### quicz 性能差距分析

quicz 当前 ~280 MB/s（8.9KB datagram），与 Linux GSO 实现仍有差距，主要原因：
...
quicz 的 Echo P50=21.5 μs 在 loopback 条件下优于多数实现的公开数据：
...
- quicz 当前 280 MB/s（8.9KB datagram，ns 精度 RTT，无快照帧处理），性能优化进行中。
- 丢包恢复和吞吐量优化是后续重点。
...
当前吞吐量瓶颈在 UDP 系统调用开销（sendto/recvfrom），增大 datagram 已从 72→280 MB/s（3.9x），下一步批量发送优化。
