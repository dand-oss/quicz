...
| 指标 | 数值 | 说明 |
|---|---|---|
| 流上传（16 MB） | **281.00 MB/s** | 线程化 client/server，CUBIC，8.9KB datagram |
| 发送 datagram 数 | ~25K | 每个 1324 B，流水线 ACK 反馈 |
| 总耗时 | ~57 ms | cwnd 增长至 4.7 MB |

## Echo 延迟（1 KB 往返，loopback）

| 百分位 | 延迟 | 说明 |
|---|---|---|
| P50 | **21.0 μs** | 1 KB 完整 QUIC 往返（加密+发送+接收+解密+回显） |
| P99 | **93.0 μs** | |
| P99.9 | **104.8 μs** | |
...
| UDP loopback（线程化） | **416.94 MB/s** | std.Io 线程化，8.9KB datagram |

## 丢包恢复（loopback + 模拟丢包）

| 丢包率 | 吞吐量 | 保持率 | 说明 |
|---|---|---|---|
| 0% | 281.00 MB/s | 100% | 基线 |
...
| **quicz** | **Zig** | **~281 MB/s** | **macOS, loopback, 8.9KB datagram, 无 GSO** | **本基准** |
...
| **quicz** | **Zig** | **21.0 μs** | **93.0 μs** | **macOS, loopback, std.Io** | **本基准** |
...
| **quicz** | **281 MB/s** | **116.63 MB/s (42%)** | **51.23 MB/s (18%)** | **CUBIC** | **本基准** |
...
| **quicz** | **416.94 MB/s** | **共享 cwnd** | **本基准** |

### quicz 性能差距分析

quicz 当前 ~281 MB/s（8.9KB datagram），与 Linux GSO 实现仍有差距，主要原因：
...
quicz 的 Echo P50=21.0 μs 在 loopback 条件下优于多数实现的公开数据：
...
- quicz 当前 281 MB/s（8.9KB datagram，ns 精度 RTT，无快照帧处理），性能优化进行中。
- 丢包恢复和吞吐量优化是后续重点。
...
当前吞吐量瓶颈在 UDP 系统调用开销（sendto/recvfrom），增大 datagram 已从 72→281 MB/s（3.9x），多流 416 MB/s，下一步批量发送优化。
