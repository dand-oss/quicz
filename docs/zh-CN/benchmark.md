...
吞吐受 ACK 时钟与单线程服务端架构限制（服务端每包处理容量 ~900 MB/s，有余量）；每包 AES-128-GCM 已硬件加速（~4.9 μs），UDP `sendto` ~3.5–4.7 μs/包，均非瓶颈。~313 MB/s 为 macOS loopback 接近实际上限；Linux 经 std.Io.Threaded sendmmsg 获得批量发送。
