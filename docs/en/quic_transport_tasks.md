...
| P7-E | External interop validation | quicz vs quic-go handshake+transfer over local UDP | PASS: quicz client handshake confirmed + transfer complete against quic-go server (3920d10) | Done |
...
| 5 | P7-E External interop | Partial | Internal pass, external pending |
...
| --- | --- |
| RFC 8999 version-independent | Done |
| RFC 9000 transport | Done |
| RFC 9001 TLS + packet protection | Done |
| RFC 9002 loss recovery + congestion control | Done |
| RFC 9221 DATAGRAM | Done |
| RFC 9368 compatible version negotiation | Done |
| RFC 9369 QUIC v2 | Done |
| HTTP/3 + QPACK | Done |

Status reconciled to the current feature matrix (2026-08); see
[feature_comparison.md](feature_comparison.md). The only remaining gaps are the
platform-only FIPS 140-3 and XDP zero-copy rows.
...
Standalone document: [feature_comparison.md](feature_comparison.md)
