...
| P7-E | External interop validation | Internal process interop with real TLS 1.3 + UDP; external requires quic-interop-runner Docker infra | Internal: PASS (handshake+echo+cleanup over real UDP, df7f492); External: blocked by quic-go Docker image availability | Blocked (infra) |
