//! quicz I/O runtime — client side (Phase 0, std.Io async model).
//!
//! A quic-go/quinn-style client over Tls13ClientEndpoint, using std.Io as the
//! I/O layer. The whole session (connect + send + recv) is designed to run as a
//! std.Io async task (Group.concurrent), so many clients can be driven on the
//! std.Io thread pool. See docs/zh-CN/io_runtime_design.md.
...
pub const EchoClient = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, io: std.Io, config: Config) !EchoClient {
    ...
    pub fn deinit(self: *EchoClient) void {
        self.client.deinit();
        self.socket.close(self.io);
    }

    /// Drive the TLS 1.3 handshake to completion (std.Io recv/send).
    pub fn connect(self: *EchoClient) !void {
        const io = self.io;
    ...
    pub fn send(self: *EchoClient, data: []const u8, fin: bool) !u64 {
    ...
    pub fn recv(self: *EchoClient, stream_id: u64, buf: []u8) !?usize {
        const io = self.io;
        var recv_buf: [max_datagram_size]u8 = undefined;
        var attempts: usize = 0;
        while (attempts < 50) : (attempts += 1) {
    ...
    /// Full echo session (connect + send + recv). Suitable for running as a
    /// std.Io async task via Group.concurrent. Returns true on a matching echo.
    pub fn runEchoSession(self: *EchoClient, payload: []const u8) !bool {
        try self.connect();
        const stream_id = try self.send(payload, true);
        var echo_buf: [4096]u8 = undefined;
        const n = try self.recv(stream_id, &echo_buf);
